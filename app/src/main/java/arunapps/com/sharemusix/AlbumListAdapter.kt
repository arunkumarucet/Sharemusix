...
class AlbumListAdapter(private val context: Context, internal var fname: ArrayList<String>, internal var artist: ArrayList<String>, internal var album: String, internal var path: ArrayList<String>) : BaseAdapter() {
    init {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        var vi = convertView
        if (convertView == null)
            vi = inflater!!.inflate(R.layout.layout_custom_list_album, parent, false)
        ...
        vi.setOnClickListener{
            ...
            DatabaseHandler(context).insertcurrent_normal(0,fname[position],artist[position],album,path[position])
            AlbumListAdapter.BackgroundTask_Database(context, position, fname, artist, album, path).execute()
        ...
        popup.setOnMenuItemClickListener { item ->
            when (item.itemId) {
                R.id.action_play_next -> {
                    ...
                    DatabaseHandler(context).insertcurrent_normal(current_position+1,fname[position],artist[position],album,path[position])
                }
                R.id.action_add_queue -> {
                    DatabaseHandler(context).insertcurrent_normal(DatabaseHandler(context).getcurrentcount(),fname[position],artist[position],album,path[position])
                ...
                R.id.action_add_playlist -> {
                    ...
                    builderSingle.setAdapter(arrayAdapter, DialogInterface.OnClickListener { dialog, which ->
                        ...
                        DatabaseHandler(context).insertplaylist(playlistname,fname[position],artist[position],album,path[position])
    ...
    class BackgroundTask_Database (val context: Context,val position: Int, val fname: ArrayList<String>, val artist: ArrayList<String>, val album: String, val path: ArrayList<String>) : AsyncTask<Void, Void, String>() {
        override fun doInBackground(vararg params: Void): String? {
            var i1 = 1
            for (i in position+1 until fname.size) {
                DatabaseHandler(context).insertcurrent_normal(i1, fname[i], artist[i], album, path[i])
                i1++
            }
            return null
        }
